...
/// Get debt trail grouped by month
/// Returns a summary of debt counts for each month before the current month
#[tauri::command]
pub async fn get_debt_trail(
    db: State<'_, PosDb>,
    end_date: String,              // YYYY-MM-DD (typically today)
) -> PosResult<Vec<DebtTrailItem>> {
    let pool = &db.0;

    // Parse end_date to get current month
    let end = end_date.parse::<chrono::NaiveDate>()
        .map_err(|e| PosError::InvalidInput(format!("Invalid date: {}", e)))?;
    let current_month = end.format("%Y-%m").to_string();

    // Get all debt goals from previous months only (exclude current month)
    ...
    .bind(format!("{}-01", current_month)) // First day of current month
    .fetch_all(pool)
    .await
    .map_err(|e| db_context("get_debt_trail", e))?;

    // Group by month (extract YYYY-MM from original_date)
    ...
    for goal in all_debt {
        if let Some(ref date) = goal.original_date {
            ...
            trail_map.entry(month).or_insert_with(Vec::new).push(goal);
        }
    }
    ...
    trail.sort_by(|a, b| a.month.cmp(&b.month));
