...
pub async fn init_pos_tables(pool: &PgPool) -> Result<(), sqlx::Error> {
    sqlx::query(POS_DDL).execute(pool).await?;
    log::info!("[POS] All PostgreSQL tables initialized");
    Ok(())
}

const POS_DDL: &str = r#"
    -- Activities: time-tracked work blocks
    ...
    -- Activity Metrics: granular metric contributions per activity
    ...
    -- Submissions: scraped competitive programming submissions
    ...
    -- Goal Metrics: quantitative targets per goal
    ...
    -- Recurring Goals: templates that generate daily goals
    ...
    -- Recurring Goal Metrics: template metrics for recurring goals
    ...
    -- Debt Goals: unverified goals that became overdue
