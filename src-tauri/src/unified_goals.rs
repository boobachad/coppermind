...
pub async fn update_unified_goal(
    ...
    if let Some(due_date) = req.due_date {
        let parsed = due_date.parse::<DateTime<Utc>>().ok();
    ...
    if let Some(urgent) = req.urgent {
        query = query.bind(urgent);
    }
    if let Some(metrics) = req.metrics {
        query = query.bind(sqlx::types::Json(metrics));
    }
    if let Some(problem_id) = req.problem_id {
        query = query.bind(problem_id);
    }
    if let Some(labels) = req.labels {
        query = query.bind(sqlx::types::Json(labels));
    }
...
pub async fn link_activity_to_unified_goal(
    ...
    // Mark goal as verified and completed
    let row = sqlx::query_as::<_, UnifiedGoalRow>(
        r#"UPDATE unified_goals 
           SET verified = TRUE,
               completed = TRUE,
