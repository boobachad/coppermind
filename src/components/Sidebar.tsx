    ...
    <div className="w-64 bg-gray-50 dark:bg-dark-bgSecondary h-screen flex flex-col flex-shrink-0">
      <div className="p-4 flex-1 overflow-hidden flex flex-col">
        ...
        <div className="mt-6 flex items-center justify-between px-2 mb-2">
          <span className="text-xs font-semibold text-gray-500 dark:text-dark-text-secondary uppercase">Recent Notes</span>
          <button onClick={createNote} className="text-gray-500 dark:text-dark-text-secondary hover:text-gray-900 dark:hover:text-dark-text-primary p-1 rounded hover:bg-gray-200 dark:hover:bg-dark-surface">
        ...
        <div className="flex-1 overflow-y-auto px-2 pb-4">
          {notes.map(note => (
            <NavLink
              key={note.id}
              to={`/notes/${note.id}`}
              className={({ isActive }) => clsx("block px-3 py-2 text-sm rounded-md truncate", isActive ? "bg-gray-200 dark:bg-dark-surface text-gray-900 dark:text-dark-text-primary" : "text-gray-600 dark:text-dark-text-secondary hover:bg-gray-100 dark:hover:bg-dark-surface")}
            >
              {note.title || 'Untitled'}
            </NavLink>
          ))}
